...
def process_model_preds(test_accession, model_type, testset_type, model_preds_path):
    """
    ...
    Args:
        ...
        model_type (str): The model type directory.
...
def run_processing(model_types, testset_types, model_preds_paths):
    """Run the full processing pipeline for all combinations of model_types, testset_types, and model_preds_paths."""
        ...
        for model_type in model_types:
            for model_preds_path in tqdm(model_preds_paths, desc="Processing predictions for model type..."):
                for test_accession in test_accessions:
                    try:
                        out_dir = (
                            f"{project_root}/data/processed_data/predictions/"
                            f"processed_predictions/{model_type}/{testset_type}/"
                            f"{model_preds_path}/{test_accession}"
                        )
                        os.makedirs(out_dir, exist_ok=True)

                        model_preds_dict, model_preds_dict_30 = process_model_preds(
                            test_accession, model_type, testset_type, model_preds_path
                        )

                        with open(f"{out_dir}/model_preds_dict.pkl", "wb") as f:
                            pickle.dump(model_preds_dict, f)
                        with open(f"{out_dir}/model_preds_dict_30.pkl", "wb") as f:
                            pickle.dump(model_preds_dict_30, f)
...
if __name__ == "__main__":
    ...
    run_processing(
        model_types="DeepCDS/model_without_errors",
        testset_types=["without_errors_300bp"],
        model_preds_paths=[
            "full_model_100_genomes_seed_42_trained_final_8M_no_dropout",
    ...
    run_processing(
        model_types="DeepCDS_A1/model_without_errors",
        testset_types=[
            "without_errors_60bp",
            "without_errors_75bp",
            "without_errors_100bp",
    ...
    run_processing(
        model_types="DeepCDS_codon_only/model_without_errors",
        testset_types=[
            "without_errors_60bp",
            "without_errors_75bp",
            "without_errors_100bp",
    ...
    run_processing(
        model_types=[
            "DeepCDS_codon_only/model_with_errors",
