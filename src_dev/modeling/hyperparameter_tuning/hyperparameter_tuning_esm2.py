...
if args.error_type == "indel_substitution":
    ...
    label_classes = 6
    wandb_project_name = "tune_esm2_errors_V2"

elif args.error_type == "substitution":
    model_dir_path_suffix = "model_with_substitution_errors"
    label_classes = 4
    wandb_project_name = "tune_esm2_substitution_errors_V2"
else:
    model_dir_path_suffix = "model_without_errors"
    label_classes = 4
    wandb_project_name = "tune_esm2_no_errors_V2"
...
wandb_project_name = "debug_codon_encoding" #MODIFY
...
def load_and_process_data(max_len):
    ...
    val_set = (val_set.groupby("accession_seq_desc_merged", group_keys=False).apply(lambda x: x.sample(frac=0.002, random_state=42))) #0.05 #MODIFY
...
def load_full_validation_set(max_len):
    ...
    ##Validate on 115k samples = 5 % of val set (0.05) following the original distribution stratified on accession and sequence type
    val_set = (val_set.groupby("accession_seq_desc_merged", group_keys=False).apply(lambda x: x.sample(frac=0.005, random_state=42))) #0.25 #MODIFY
...
def objective(trial, train_data, val_data, val_loader_full, sequence_types, seq_type_desc_fracs, label_classes):
    ...
    eval_every_n_steps = 100 #MODIFY to 4000 -> 4000 * 32 samples between each validation
...
file_path = f'{input_data_dir_path}/hyperparameter_configs/esm2_8m_hyperparameters_REMOVE.yaml'
