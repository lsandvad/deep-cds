...
if device_type == "mps":
    ...
    input_data_dir_path = "/tmp/nrt204/FragmentPredictor/data/processed_data/model_data/shared_crf/model_without_errors"
    num_workers_cpu = 4
    pin_memory = True
...
wandb_project_name = "tune_shared_full_model_no_errors" #MODIFY
...
def load_and_process_data(max_len):
    ...
    ##Validate on 115k samples = 5 % of val set (0.05) following the original distribution stratified on accession and sequence type
    #train_set = (train_set.groupby("accession_seq_desc_merged", group_keys=False).apply(lambda x: x.sample(frac=0.02, random_state=42))) ##DELETE
