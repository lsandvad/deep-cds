...
if device_type == "mps":
    ...
    input_data_dir_path = "/tmp/nrt204/FragmentPredictor3/data/processed_data/model_data/shared_crf/model_with_errors" #TEST ON SCARB CLUSTER
    num_workers_cpu = 4
    pin_memory = True
...
os.makedirs(f"../../../data/processed_data/model_data/shared_crf/model_with_errors/models_optuna/", exist_ok=True)

#dir in wandb to place runs
wandb_project_name = "experiments" #"tune_shared_full_model_opt" #MODIFY!

max_aa_len = 200 #MODIFY!
...
def load_and_process_data(max_len):
    ...
    val_set = (val_set.groupby("accession_seq_desc_merged", group_keys=False).apply(lambda x: x.sample(frac=0.001, random_state=42))) #MODIFY -> 0.05
...
def load_full_validation_set(max_len):
    ...
    #Create a combined stratification label for accession and sequence type
    val_set["accession_seq_desc_merged"] = val_set["accession"].astype(str) + "_" + val_set["seq_desc"].astype(str) #DELETE

    ##Validate on 115k samples = 5 % of val set (0.05) following the original distribution stratified on accession and sequence type
    val_set = (val_set.groupby("accession_seq_desc_merged", group_keys=False).apply(lambda x: x.sample(frac=0.001, random_state=42))) #DELETE
...
class LinearChainCRF(nn.Module):
    ...
    def __init__(self, 
        ...
        #Get class weights as tensors
        if class_weights is not None:
            self.register_buffer('class_weights', torch.FloatTensor(class_weights))
        else:
            self.register_buffer('class_weights', torch.ones(num_class_labels))
        ...
        with torch.no_grad():
            self.crf.transitions[~self.biologically_valid_mask] = -10  # forbidden → -10
            self.crf.transitions[self.biologically_valid_mask] = 1    # allowed → 1. #MODIFY?
    ...
    def forward(self, logits, attention_mask, labels=None, sequence_class=None):
        """
        ...
        if labels is not None:
            ...
            if sequence_class is not None:
                seq_weights = self.class_weights[sequence_class]  # Use stored weights
                loss = -(log_likelihood * seq_weights).mean()
            else:
...
class CDSPredictor(nn.Module):
    ...
    def forward(self, encoded_seqs_nt_rf0, x_aa_rf0, attention_mask_aa_rf0, 
                      ...
                      labels=None, sequence_class=None):
        """
...
def initialize_model(device, num_layers, n_attention_heads, dropout_rate_1, dropout_rate_2, act_function, class_weights):
    """
...
def training_iteration(i, batch, scaler, model, optimizer, device, epoch_losses, seq_type_to_idx):
    """
    ...
    #Convert string sequence types to indices
    seq_type_strings = batch['seq_desc']  #List of strings like ['non-coding', 'coding', ...]
    sequence_classes = torch.tensor([seq_type_to_idx[s] for s in seq_type_strings], 
    ...
    if scaler is not None:
        with autocast("cuda"):
            outputs = model(inputs_nt_rf0, inputs_aa_rf0, attention_mask_aa_rf0,
                          ...
                          sequence_class=sequence_classes)
            loss = outputs['loss']
    else:
...
def objective(trial, train_data, val_data, val_loader_full, sequence_types, seq_type_desc_fracs):
    ...
    weight_class_scalers = [0, 0.1, 0.25, 0.5]
    ...
    lr_esm2 = trial.suggest_float('lr_esm2', 1e-6, 1e-5, log=True)
    ...
    weight_class_scaler = trial.suggest_categorical('weight_class_scaler', weight_class_scalers)

    batch_size = 32

    # Define mapping ONCE at the top of your training script
    seq_type_to_idx = {
        'non-coding': 0,
        'coding_with_substitutions': 1,
        'transition_indel': 2,
        'transition_start': 3,
        'transition_stop': 4,
        'coding': 5,
        'transition_start_stop': 6
    }

    # Calculate weights as dict first
    class_weights_dict = {}
    for seq_type, freq in seq_type_desc_fracs.items():
        weight = (1.0 / freq) ** weight_class_scaler
        class_weights_dict[seq_type] = weight

     # Convert to list with proper indexing
    class_weights_list = [0.0] * 7  # Create list
    for seq_type, idx in seq_type_to_idx.items():
        class_weights_list[idx] = class_weights_dict[seq_type]

    print("\nClass weights:")
    for seq_type, idx in sorted(seq_type_to_idx.items(), key=lambda x: x[1]):
        print(f"  {seq_type:30s} (idx {idx}): {class_weights_list[idx]:.3f}")
    ...
    print(f"depth_transformer_encoder_blocks: {depth_transformer_encoder_blocks}\n \
            ...
            act_function: {act_function}\n \
            weight_class: {weight_class_scaler}")
    ...
    eval_every_n_steps = 100 #8000 #MODIFY to 8000
    ...
    threshold_patience = 8  #Number of evaluations with no improvement to wait before stopping
    counter_patience = 0
    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            epoch_losses = training_iteration(i, batch, scaler, model, optimizer, device, epoch_losses, seq_type_to_idx)
            ...
            if step % eval_every_n_steps == 0 and step > 0:
                ...
                if val_avg_loss < best_val_loss:
                    ...
                    torch.save(model.state_dict(), f"../../../data/processed_data/model_data/shared_crf/model_with_errors/models_optuna/trial{trial.number}_trained.pth")
                    counter_patience = 0
                else:
                    counter_patience += 1
    ...
    model.load_state_dict(torch.load(f"../../../data/processed_data/model_data/shared_crf/model_with_errors/models_optuna/trial{trial.number}_trained.pth", map_location=device), strict=False)
...
model_config = {
    ...
    "weight_class_scaler": study.best_params["weight_class_scaler"],
...
file_path = f'../../../data/processed_data/model_data/shared_crf/model_with_errors/hyperparameter_configs/esm2_8m_nt_hyperparameters.json'
