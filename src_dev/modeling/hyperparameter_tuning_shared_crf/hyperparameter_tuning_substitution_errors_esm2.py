...
if device_type == "mps":
    ...
    input_data_dir_path = "/tmp/nrt204/FragmentPredictor/data/processed_data/model_data/shared_crf/model_with_substitution_errors" #TEST ON SCARB CLUSTER
    num_workers_cpu = 4
    pin_memory = True
...
os.makedirs(f"{input_data_dir_path}/models_optuna_esm2/", exist_ok=True)
os.makedirs(f"{input_data_dir_path}/hyperparameter_configs/", exist_ok=True)
#dir in wandb to place run
wandb_project_name = "tune_shared_esm2_substitution_errors" #MODIFY
...
def load_and_process_data(max_len):
    ...
    ##Validate on 115k samples = 5 % of val set (0.05) following the original distribution stratified on accession and sequence type
    #train_set = (train_set.groupby("accession_seq_desc_merged", group_keys=False).apply(lambda x: x.sample(frac=0.001, random_state=42))) ##DELETE
...
def load_full_validation_set(max_len):
    ...
    ##Validate on 115k samples = 5 % of val set (0.05) following the original distribution stratified on accession and sequence type
    #val_set = (val_set.groupby("accession_seq_desc_merged", group_keys=False).apply(lambda x: x.sample(frac=0.0001, random_state=42))) #0.05 #MODIFY
...
def objective(trial, train_data, val_data, val_loader_full, sequence_types, seq_type_desc_fracs):
    ...
    eval_every_n_steps = 4000 #MODIFY to 4000 -> 4000 * 32 samples between each validation
        ...
        for i, batch in enumerate(train_loader):
            ...
            if step % eval_every_n_steps == 0 and step > 0:
                ...
                with torch.no_grad():
                    for counter, val_batch in enumerate(val_loader):
                        ...
                        if counter % 30 == 0:
                            ...
                            del v_inputs_aa_rf0, v_attention_mask_aa_rf0, v_inputs_aa_rf1, v_attention_mask_aa_rf1, v_inputs_aa_rf2, v_attention_mask_aa_rf2, 
                ...
                if val_avg_loss < best_val_loss:
                    ...
                    torch.save(model.state_dict(), f"{input_data_dir_path}/models_optuna_esm2/trial{trial.number}_trained.pth")
                    counter_patience = 0
                else:
                    counter_patience += 1
    ...
    model.load_state_dict(torch.load(f"{input_data_dir_path}/models_optuna_esm2/trial{trial.number}_trained.pth", map_location=device), strict=False)
...
file_path = f'{input_data_dir_path}/hyperparameter_configs/esm2_8m_hyperparameters.yaml'
