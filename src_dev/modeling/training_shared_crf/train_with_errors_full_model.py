...
class SeqDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        item = {
            ...
            "label_encodings": torch.from_numpy(self.label_encodings[idx])
            if isinstance(self.label_encodings[idx], np.ndarray)
            else torch.tensor(self.label_encodings[idx], dtype=torch.float32),
    ...
    for i, batch in enumerate(train_loader):
        ...
        if step % eval_every_n_steps == 0 and step > 0:
            ...
            if val_avg_loss < best_val_loss:
                ...
                torch.save(
                    model.state_dict(),
                    f"{models_output_dir_path}/full_model_{dataset_size}_seed_{args.seed}_trained.pth"
                )  # modify #ADD THIS
                counter_patience = 0
            else:
                counter_patience += 1
            ...
            if val_times_counter > freeze_esm_validations:
                ...
                print(f"ESM-2 parameters changed by: {relative_change:.6e} ({relative_change * 100:.4f}%)")
