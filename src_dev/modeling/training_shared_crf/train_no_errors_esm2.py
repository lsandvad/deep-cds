...
parser.add_argument('--dataset_size', type=str, default='100_genomes', 
...
parser.add_argument('--healthtech_cluster', type = bool, default=False,
                    help = 'Whether running on HealthTech cluster (default: False)')
parser.add_argument('--scarb_cluster', type = bool, default=False,
...
if args.healthtech_cluster:
    input_data_dir_path = "../../../data/processed_data/model_data/shared_crf/model_without_errors"
    num_workers_cpu = 4
    pin_memory = True
    # Use argparse values
    ...
    os.makedirs(f"/home/projects/DeepCDStmp/data/processed_data/model_data/shared_crf/model_without_errors/models/", exist_ok=True) #While projects2 fails
    models_output_dir_path = f"/home/projects/DeepCDStmp/data/processed_data/model_data/shared_crf/model_without_errors/models/"


elif args.scarb_cluster:
    input_data_dir_path = "/tmp/nrt204/FragmentPredictor/data/processed_data/model_data/shared_crf/model_without_errors" #SCARB cluster
    num_workers_cpu = 4
    pin_memory = True
    # Use argparse values
...
os.makedirs(f"{input_data_dir_path}/data/processed_data/model_data/shared_crf/model_without_errors/models/", exist_ok=True)

#dir in wandb to place runs
wandb_project_name = "train_no_errors_esm2" 
...
class SeqDataset(torch.utils.data.Dataset):
    ...
    """
    def __init__(self, aa_encodings_rf0, labels_rf0, 
                 aa_encodings_rf1, labels_rf1, 
                 aa_encodings_rf2, labels_rf2, 
                 label_encodings,
                 seq_desc):
...
def encode_data(processed_samples_df, max_len, tokenizer=None):
    ...
    dataset = SeqDataset(encodings_aa["rf0"], labels["rf0"],
                         encodings_aa["rf1"], labels["rf1"],
                         encodings_aa["rf2"], labels["rf2"],
...
def load_and_process_data(max_len, dataset_size = dataset_size):
    ...
    val_set = (val_set.groupby("accession_seq_desc_merged", group_keys=False).apply(lambda x: x.sample(frac=0.25, random_state=42))) #DELETE
...
class TransformerEncoderBlock(nn.Module):
    ...
    def __init__(self,
        ...
        hidden_size_merged = hidden_size #hidden_size for amino acid representation from ESM2
...
class CDSPredictor(nn.Module):
    ...
    def __init__(self,
        ...
        #Linear layer to combine outputs from the 3 reading frames (3 * 4 logits -> num_encoded_labels)
        self.linear_transform = nn.Linear(3*4, num_encoded_labels)
    ...
    def forward(self, x_aa_rf0, attention_mask_aa_rf0, 
                      x_aa_rf1, attention_mask_aa_rf1, 
                      x_aa_rf2, attention_mask_aa_rf2, 
                      labels=None):
        """
        ...
        logits_rf2 = self.TransformerEncoderBlock(encoded_embeddings_aa=encoded_embeddings_aa_rf2, trimmed_attention_mask=trimmed_attention_mask_rf2) #output: [codon_seq_len, C]
...
def training_iteration(i, batch, scaler, model, optimizer, device, epoch_losses, warmup_state=None):
    ...
    if scaler is not None:
        with autocast("cuda"):
            outputs = model(inputs_aa_rf0, attention_mask_aa_rf0,
                          inputs_aa_rf1, attention_mask_aa_rf1,
                          inputs_aa_rf2, attention_mask_aa_rf2,
    ...
    if i % 1000 == 0:
        ...
        del outputs, loss
        del inputs_aa_rf0, attention_mask_aa_rf0
        del inputs_aa_rf1, attention_mask_aa_rf1
...
#Define hyperparameters (tuned values from previous tuning runs)
depth_transformer_encoder_blocks = 8
n_attention_heads = 4
dropout_rate_1 = 0.3
dropout_rate_2 = 0.2
lr_scratch = 0.00005102916182097042
lr_esm2 = 0.00000312659891982138
act_function = 'relu'
transition_weight = -1.3648789111020785
...
wandb.init(project=wandb_project_name, 
           config={
               ...
               name = f"train_{dataset_size}") #MODIFY
...
eval_every_n_steps = 6000 #6000  #Validate every 6000 batches = 6000 * 32 samples #MODIFY 
        ...
        if step % eval_every_n_steps == 0 and step > 0:
            ...
            with torch.no_grad():
                for counter, val_batch in enumerate(val_loader):
                    ...
                    if scaler is not None:
                        with autocast("cuda"):
                            v_outputs = model(v_inputs_aa_rf0, v_attention_mask_aa_rf0, 
                                            v_inputs_aa_rf1, v_attention_mask_aa_rf1, 
                                            v_inputs_aa_rf2, v_attention_mask_aa_rf2, 
                                            v_encoded_labels)
                    ...
                    if val_times_counter % 5 == 0:
                        ...
                        if counter == 0:
                            show_examples(v_encoded_labels, padding_mask, logits_for_metrics, 
                                        seq_descs_batch, mapping_dict_to_class, model, device, valid_mask)
            ...
            if val_avg_loss < best_val_loss:
                ...
                torch.save(model.state_dict(), f"{input_data_dir_path}/models/esm2_8m_{dataset_size}_trained.pth") #modify
                counter_patience = 0
            else:
                counter_patience += 1
