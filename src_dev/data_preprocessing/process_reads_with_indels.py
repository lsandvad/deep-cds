...
def convert_complement_coordinates(refseq_accession, cds_coords_complement, cds_coords_uncertain_complement):
    ...
    genome_files = os.listdir(f"../../data/raw_data/genome_data/{refseq_accession}/")
    genome_fasta_file = [file for file in genome_files if file.startswith(refseq_accession)][0]
    genome_filename = f"../../data/raw_data/genome_data/{refseq_accession}/{genome_fasta_file}"
...
def extract_CDS(refseq_accession):
    ...
    try:
        with open("../../data/raw_data/genome_data/"+genbank_accession+"/genomic.gff", "r") as gff_file_genbank:
            for line in range(7):
            ...
            for line in gff_file_genbank:
                ...
                if "\tCDS\t" in line and "\t+\t" in line:
                    ...
                    if "pseudo=true" in line.lower() or "product=hypothetical protein" in line.lower() or "partial=true" in line.lower() or "ab initio prediction" in line.lower() or "note=programmed frameshift" in line.lower():
    ...
    try:
        with open("../../data/raw_data/genome_data/"+refseq_accession+"/genomic.gff", "r") as gff_file_refseq:
            for line in range(9):
    ...
    return cds_coords_template, cds_coords_uncertain_template, \
           cds_coords_complement, cds_coords_uncertain_complement
...
def proces_reads_to_dict(accession, seqs_len, cds_coords_uncertain, strand, partition, error_rates):
    ...
    with pysam.AlignmentFile(f"../../data/processed_data/simulated_reads/{partition}/{error_rates}/{strand}/alignments/{accession}_alignments.bam", "rb") as bam_infile:
        for read in bam_infile:
            ...
            reads_information_dict[assembly][read_id] = {
                "CIGAR": CIGAR,
    ...
    for start_idx in sorted_indices:
        ...
        while True:
            ...
            for next_idx in range(n):
                ...
                if (0 <= gap <= max_gap and
                    next_frame != last_frame and
                    is_indel_transition(last_end, next_start, last_frame, next_frame) and
                    gap < best_gap):
                    best_next = next_idx
                    best_gap = gap
...
def mark_intervals(labels_rf0_nt, labels_rf1_nt, labels_rf2_nt,
                   labels_rf0, labels_rf1, labels_rf2):
    """ 
...
def quality_check_CDS_fragments(coding_seqs_all_read, accession):
    ...
    if coding_seqs_all_read != []:
        ...
        for coding_seq in coding_seqs_all_read: 
            ...
            seq_found = False
            for record in SeqIO.parse(f"../../data/raw_data/genome_data/{accession}/protein.faa", "fasta"):
                prot_seq = record.seq
            ...
            if not seq_found:
                try: 
                    for record in SeqIO.parse(f"../../data/raw_data/genome_data/{genbank_accession}/protein.faa", "fasta"):
                        prot_seq = record.seq
                        if coding_seq in prot_seq:
                            seq_found = True
                            break
    ...
    try:
        ...
        for read_id in read_ids:
            ...
            if write_read: 
                ...
                cds_overlaps_read, indel_cds_connect = mark_intervals(rf0_labels_nt, rf1_labels_nt, rf2_labels_nt,
...
def run_pipeline(seqs_len, accession, partition, error_rates):
    ...
    attributes = ['read_name', 'read', 'cds_coords', 'cds_fragments_connection', 'start_coord', 'assembly_id', 'CIGAR', 'MD:Z', 
            'indel_positions', 'accession', 'rf0_seq', 'rf0_labels', 
            'rf1_seq', 'rf1_labels', 'rf2_seq', 'rf2_labels', 'strand']
    ...
    for record in SeqIO.parse(genome_filename, "fasta"):
        ...
        processed_reads_df, reads_correct, reads_wrong = process_strand_reads(assembly_id, assembly_seq, accession, seqs_len, cds_coords_template, reads_information_dict_template, processed_reads_df, reads_correct, reads_wrong, "+")
        processed_reads_df, reads_correct, reads_wrong = process_strand_reads(assembly_id, assembly_seq_rev, accession, seqs_len, cds_coords_complement, reads_information_dict_complement, processed_reads_df, reads_correct, reads_wrong, "-")
    
    #check if directories exist, if not create them
    if not os.path.exists(f"../../data/processed_data/reads_processed/{partition}/{error_rates}/csv/"):
        os.makedirs(f"../../data/processed_data/reads_processed/{partition}/{error_rates}/csv/")
    if not os.path.exists(f"../../data/processed_data/reads_processed/{partition}/{error_rates}/fasta/"):
        os.makedirs(f"../../data/processed_data/reads_processed/{partition}/{error_rates}/fasta/")
    ...
    processed_reads_df.to_csv(f"../../data/processed_data/reads_processed/{partition}/{error_rates}/csv/{accession}.csv.gz", compression='gzip')

    #Create fasta file 
    with gzip.open(f"../../data/processed_data/reads_processed/{partition}/{error_rates}/fasta/{accession}.fasta.gz", "wt") as fasta_out:  # 'wt' = write text mode
        for _, row in processed_reads_df.iterrows():
            fasta_out.write(
                f">{row['read_name']}|{row['strand']}|{row['assembly_id']}|{row['cds_coords']}|{row['indel_positions']}\n"
                f"{row['read']}\n"
            )
...
#accessions_processed = os.listdir("../../data/processed_data/reads_processed/train_val/with_errors/csv/")
...
#accessions = ["GCF_000012765.1"] #translation table 4
#accessions = ['GCF_001277215.2', 'GCF_000006765.1'] #tests 

for error_rates in ["with_errors_1.25e-05i_0.01s_300bp",
    ...
    if __name__ == "__main__":
        ...
        if "60bp" in error_rates:
            seqs_len = 60
        elif "100bp" in error_rates:
            seqs_len = 100
        elif "150bp" in error_rates:
            seqs_len = 150
        elif "300bp" in error_rates:
            seqs_len = 300
        elif "600bp" in error_rates:
            seqs_len = 600
        elif "1000bp" in error_rates:
            seqs_len = 1000

        assert seqs_len != 0, "Something went wrong with interpreting sequence length."
