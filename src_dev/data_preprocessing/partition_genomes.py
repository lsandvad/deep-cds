...
"""
Make lists of genome IDs that go into train, val and test partitions
"""

def partition_dataset(tax_info_genomes_df, testset_accessions, random_seed=42):
    """
    ...
    """
    def __calculate_family_stats__(domain_df, family_groups):
        family_stats = []
        for family, indices in family_groups.items():
            ...
            family_stats.append({
                ...
                'indices': indices,
                'count': len(indices),
                'mean_gc': family_data['gc_content'].mean()
            })

        return pd.DataFrame(family_stats).sort_values('mean_gc').reset_index(drop=True)

    def __select_validation_families__(family_stats_df, family_groups, domain_name):
    ...
    train_df = pd.concat([
        archaea_df.loc[archaea_train_indices],
        bacteria_df.loc[bacteria_train_indices]
    ])
    
    val_df = pd.concat([
        archaea_df.loc[archaea_val_indices],
        bacteria_df.loc[bacteria_val_indices]
    ])
    ...
    return {
        'train': train_df['accession'].tolist(),
        'val': val_df['accession'].tolist(),
        'test': test_df['accession'].tolist(),
        'train_df': train_df,
        'val_df': val_df,
        'test_df': test_df
    }
...
testset_accessions_archaea = {"GCF_000011125.1", #56.3
...
testset_accessions_bacteria = {"GCF_000007365.1", #25.3
