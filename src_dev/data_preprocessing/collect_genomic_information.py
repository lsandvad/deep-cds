...
def process_taxonomic_and_genomic_info() -> None:
    ...
    """
    os.makedirs(f"{mount_point}/data/processed_data/dataset_information", exist_ok=True)
    genomes_data_dict = dict()

    with open(f"{mount_point}/data/processed_data/taxonomy/genomes_tax_info.tab", "r") as infile_tax_data:
        ...
        for line in infile_tax_data:
            ...
            if genomes_data_dict[accession]["family"] is None:
                if os.path.exists(f"{mount_point}/data/raw_data/genome_data/{accession}"):
                    print(f"Data with accession: {accession} removed due to lack of family-level classification.")
                    shutil.rmtree(f"{mount_point}/data/raw_data/genome_data/{accession}")

                genbank_accession = accession.replace("GCF", "GCA")
                if os.path.exists(f"{mount_point}/data/raw_data/genome_data/{genbank_accession}"):
                    print(f"Data with accession: {genbank_accession} removed due to lack of family-level classification.")
                    shutil.rmtree(f"{mount_point}/data/raw_data/genome_data/{genbank_accession}")
            ...
            # If accession is classified on family level, calculate and add statistical metadata
                ...
                genome_files = os.listdir(f"{mount_point}/data/raw_data/genome_data/{accession}/")
                genome_fasta_file = [file for file in genome_files if file.startswith(accession)][0]
                genome_filename = f"{mount_point}/data/raw_data/genome_data/{accession}/{genome_fasta_file}"
                ...
                with open(f"{mount_point}/data/raw_data/genome_data/{accession}/genomic.gff", "r") as infile_genomic_gff:
    ...
    genomes_data_df.to_csv(f"{mount_point}/data/processed_data/dataset_information/genomes_info.csv", sep=",", encoding="utf-8")
